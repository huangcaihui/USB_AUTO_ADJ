...
void USBH_Process(USB_OTG_CORE_HANDLE *pdev , USBH_HOST *phost)
{
  ...
  case HOST_IDLE :
    //LCD_UsrLog("HOST_IDLE 00 \n");
    if (HCD_IsDeviceConnected(pdev))  
    {
        LCD_UsrLog("HOST_IDLE 11 \n");
  ...
  case HOST_DEV_ATTACHED :
    LCD_UsrLog("HOST_DEV_ATTACHED 11 \n");
  ...
  case HOST_ENUMERATION:     
    /* Check for enumeration status */  //进入枚举过程
    LCD_UsrLog("HOST_ENUMERATION 11 \n");
    if ( USBH_HandleEnum(pdev , phost) == USBH_OK)
    { 
      /* The function shall return USBH_OK when full enumeration is complete */
      LCD_UsrLog("USBH_HandleEnum() = USBH_OK 11 \n");
  ...
  case HOST_USR_INPUT:    //用户输入确认，这个函数返回用户的确认的确认信息
    LCD_UsrLog("HOST_USR_INPUT 11111 \n");
    /*The function should return user response true to move to class state */
    if ( phost->usr_cb->UserInput() == USBH_USR_RESP_OK)
    {
        LCD_UsrLog("HOST_USR_INPUT 222222 \n");
      if((phost->class_cb->Init(pdev, phost))\
        == USBH_OK)
      {
        LCD_UsrLog("HOST_USR_INPUT 333333 \n");
  ...
  case HOST_CLASS_REQUEST:  
    ...
    LCD_UsrLog("HOST_CLASS_REQUEST 11111 \n");
     if(status == USBH_OK)
     {
        LCD_UsrLog("HOST_CLASS_REQUEST 2222222 \n");
       phost->gState  = HOST_CLASS;
     }  
     
     else
     {
        LCD_UsrLog("HOST_CLASS_REQUEST 3333333 \n");
  ...
  case HOST_CLASS:  //开始类的状态机，在这个函数里面调用用户自定义的应用函数。  
    ...
    LCD_UsrLog("HOST_CLASS() = USBH_OK 22 \n");
...
static USBH_Status USBH_HandleEnum(USB_OTG_CORE_HANDLE *pdev, USBH_HOST *phost)
{
  ...
  {
  case ENUM_IDLE:  
    LCD_UsrLog("ENUM_IDLE 44444 \n"); 
    /* Get Device Desc for only 1st 8 bytes : To get EP0 MaxPacketSize */
    if ( USBH_Get_DevDesc(pdev , phost, 8) == USBH_OK)
    {
        LCD_UsrLog("ENUM_IDLE 555555 \n"); 
  ...
  case ENUM_GET_FULL_DEV_DESC:  
    ...
    if ( USBH_Get_DevDesc(pdev, phost, USB_DEVICE_DESC_SIZE)\
      == USBH_OK)
    {
        LCD_UsrLog("ENUM_GET_FULL_DEV_DESC 44444 \n"); 
  ...
  case ENUM_DEV_CONFIGURED:
    ...
    LCD_UsrLog("ENUM_GET_FULL_DEV_DESC 55555 \n"); 
    Status = USBH_OK;
    break;
