                                   ...
                                   void   *phost)
{
  ...
  LCD_UsrLog("USBH_HID_Handle 11111 \n");
  switch (HID_Machine.state)
  {
    
  case HID_IDLE:
    LCD_UsrLog("USBH_HID_Handle 2222222222 \n");
    HID_Machine.cb->Init();
    HID_Machine.state = HID_SYNC;
    
  case HID_SYNC:
    LCD_UsrLog("USBH_HID_Handle 3333333 \n");
  ...
  case HID_GET_DATA:
      LCD_UsrLog("USBH_HID_Handle 44444444444 \n");
  ...
  case HID_POLL:
    LCD_UsrLog("USBH_HID_Handle 5555555 \n");
                                                  ...
                                                  uint16_t length)
{
  ...
  USBH_Status status;
  LCD_UsrLog("USBH_HID_Handle 6666666666 \n");
                                            ...
                                            uint16_t length)
{
  ...
  USBH_Status status;
  LCD_UsrLog("USBH_HID_Handle 7777777777 \n");
  status = USBH_GetDescriptor(pdev, 
                              phost,
                              USB_REQ_RECIPIENT_INTERFACE
                                | USB_REQ_TYPE_STANDARD,                                  
...
static USBH_Status USBH_Set_Idle (USB_OTG_CORE_HANDLE *pdev,
                                  ...
                                  uint8_t reportId)
{
  LCD_UsrLog("USBH_HID_Handle 88888888888888 \n");
...
USBH_Status USBH_Set_Report (USB_OTG_CORE_HANDLE *pdev, 
                                 USBH_HOST *phost,
                                    ...
                                    uint8_t* reportBuff)
{
  LCD_UsrLog("USBH_HID_Handle 8999999999999999 \n");
  phost->Control.setup.b.bmRequestType = USB_H2D | USB_REQ_RECIPIENT_INTERFACE |\
    USB_REQ_TYPE_CLASS;
                                     ...
                                     uint8_t protocol)
{
  ...
  LCD_UsrLog("USBH_HID_Handle 1010101100 \n");
  phost->Control.setup.b.bmRequestType = USB_H2D | USB_REQ_RECIPIENT_INTERFACE |\
    USB_REQ_TYPE_CLASS;
...
static void  USBH_ParseHIDDesc (USBH_HIDDesc_TypeDef *desc, uint8_t *buf)
{
  LCD_UsrLog("USBH_HID_Handle 1212121212 \n");
